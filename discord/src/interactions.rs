...
use crate::disc_objects::{AppCommandOptionType, ApplicationCommandType, ChannelType, Snowflake};
use crate::discord;
...
pub type PinnedInteraction =
    Box<dyn Fn(disc_objects::Interaction, bot::BotClient) -> BoxFuture<'static, ()> + Send + Sync>;
pub type AppCommandMap =
    Arc<RwLock<HashMap<disc_objects::ApplicationCommandType, HashMap<String, InteractionCommand>>>>;
...
impl AppCommandOption {
    ...
    pub fn add_choice(mut self, name: &str, value: AppCommandValue) -> Self {
        let value = match self.app_command_option_type {
            ...
            | AppCommandOptionType::Number => value,
            _ => panic!(
                "You can only add choice if option type is String, Integer or Number (double)"
            ),
        };
    ...
    pub fn add_choice_from_struct(mut self, choice: AppCommandChoice) -> Self {
        let choice = match self.app_command_option_type {
            AppCommandOptionType::String
            | AppCommandOptionType::Integer
            | AppCommandOptionType::Number => choice,
            _ => panic!(
                "You can only add choice if option type is String, Integer or Number (double)"
            ),
        };
    ...
    pub fn add_option(mut self, option: AppCommandOption) -> Self {
        let option = match self.app_command_option_type {
            ...
            _ => {
                panic!("You can only add options if option type is subcommand or subcommand group.")
            }
        };
...
impl crate::bot::Bot {
    pub async fn add_app_command(
        ...
        let interaction_command = InteractionCommand::new(command.clone(), function, guild_ids);

        let command_type = command.app_command_type.clone();
        let command_type_exists = self
            .interaction_map
            .read()
            .await
            .get(&command_type)
        ...
        if command_type_exists {
            self.interaction_map
                ...
                .unwrap()
                .insert(command.name, interaction_command);
        } else {
            ...
            self.interaction_map
                .write()
                .await
                .insert(command_type, nested_map);
        }
    }

    pub async fn create_command(client: bot::BotClient, guild_map: HashMap<Snowflake, Vec<Value>>) {
        let base = format!("/applications/{}/", client.read().await.application_id);
        ...
        for (guild_id, app_commands) in guild_map {
            ...
            let overwrite =
                discord::HttpRequest::string_new(base.clone() + &extension, client.clone()).await;

            println!(
                "{:#?}",
                overwrite.put(serde_json::Value::Array(app_commands)).await
            );
        }
    }
    ...
    pub async fn register_commands(client: bot::BotClient, interaction_map: AppCommandMap) {
        ...
        for application_command_type in interaction_map.read().await.keys() {
            for (_name, command) in interaction_map
                .read()
                .await
                .get(application_command_type)
                .unwrap()
                .iter()
            {
    ...
    pub async fn handle_interaction(
        interaction: Option<discord::GatewayEvent>,
        ...
        if interaction.data.is_some() {
            if interaction_map.is_some() {
                match interaction.interaction_type {
                    disc_objects::InteractionType::ApplicationCommand => {
                        bot::Bot::handle_app_command(
                            interaction,
                            interaction_map.unwrap(),
                            client.clone(),
                        )
                        .await
                    }
                    _ => panic!(
                        "Celestialcord only supports the application command type right now!"
                    ),
                }
            } else {
    ...
    pub async fn handle_app_command(
        interaction: disc_objects::Interaction,
        interaction_map: AppCommandMap,
        client: BotClient,
    ) {
        interaction_map
            .read()
            .await
            .get(&interaction.clone().data.unwrap().interaction_data_type)
            .unwrap()
            .get(&interaction.clone().data.unwrap().name)
            .unwrap()
            .function
            .as_ref()(interaction, client.clone())
        .await;
    }
}

impl disc_objects::Interaction {
    fn check_ephemeral(ephemeral: bool) -> Option<i32> {
        if ephemeral == true {
            Some(1 << 6)
        } else {
            None
    ...
    pub async fn deferRespond(&self, client: BotClient, ephemeral: bool) {
        let ephemeral = disc_objects::Interaction::check_ephemeral(ephemeral);
        ...
        let extension = format!("/interactions/{}/{}/callback", self.id.clone(), self.token);

        let request = discord::HttpRequest::string_new(extension, client.clone()).await;
        request.post(content).await;
    }

    pub async fn respond(&self, message: disc_objects::ReplyMessage, client: BotClient) {
        let ephemeral = disc_objects::Interaction::check_ephemeral(message.ephemeral);
    ...
    pub async fn follow_up(
        &self,
        message: disc_objects::ReplyMessage,
        client: BotClient,
        ephemeral: bool,
    ) {
    }

    pub async fn edit_response(&self, message: disc_objects::ReplyMessage, client: BotClient) {
        let content = json!({
            "content": message.content,
            "tts": message.tts,
            "embeds": message.embeds,
        ...
        let extension = format!(
            ...
            client.read().await.application_id,
            self.token
        );

        let request = discord::HttpRequest::string_new(extension, client.clone()).await;
        request.patch(content).await;
    }

    pub async fn get_response(&self, client: BotClient) {}

    pub async fn delete_response(&self, client: BotClient) {}
}
